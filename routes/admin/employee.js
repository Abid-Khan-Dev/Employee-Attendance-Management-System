...
router.post('/add', async (req, res) => {
  try {
    ...
    const { firstName, lastName, username, email, password, joiningDate, shift, dob, phone, gender, department, statusEmp } = req.body;
...
router.get('/edit/:id', async (req, res) => {
  try {
    ...
    const department = await Department.find();
    const employee = await User.findById(req.params.id).populate('department').populate('shift');
...
router.post('/edit/:id', async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ _id: req.params.id }, {firstName,
      lastName,
      username,
      email,
      password: hashPassword,
